...
// НЕ ПОДКЛЮЧАТЬ
// НЕ ИМПОРТИРОВАТЬ
// Только эталон старой функциональности SecretTalk
...
try {
await db.query(
INSERT INTO users (telegram_id, username, first_name)   VALUES ($1, $2, $3)   ON CONFLICT (telegram_id)   DO UPDATE SET   username = EXCLUDED.username,   first_name = EXCLUDED.first_name,   last_seen = CURRENT_TIMESTAMP,
[
msg.from.id,
...
быть живой виртуальной собеседницей, с которой человеку приятно проводить время, получить наслаждение и кайф и к которой хочется возвращаться.
  },   ...chatHistory[msg.chat.id]   ]   },   {   headers: {   Authorization:Bearer ${process.env.OPENROUTER_API_KEY}`,
